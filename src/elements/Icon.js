...
// Find a more eloquent way of prevening render of styled baseIcon if invalid 'type' is passed in as prop
const baseIcon = ({ className, type, altText }) => {
  const typeLC = type && type.toLowerCase();
  return iconMap.hasOwnProperty(typeLC)  ?
    <img className={className} src={iconMap[typeLC].src} alt={ altText || iconMap[typeLC].altText} /> : null

};

const Icon = styled(baseIcon)(props =>
  iconMap.hasOwnProperty(props.type && props.type.toLowerCase()) ?
  {

    backgroundSize: 'contain',
      height: `${props.height}px`,
    width: `${(iconMap[props.type.toLowerCase()].widthRatio || 1) * props.height}px`,
